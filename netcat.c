...
#ifdef INET6
HINF6 *gethost6poop(name, numeric) char *name;
USHORT numeric;
{
  struct hostent *hostent;
  ...
  if (!inet_pton(AF_INET6, name, &iaddr))
  { /* here's the great split: names... */
    if (numeric)
      bail("Can't parse %s as an IP address", name);
...
/* fall thru to outbound connects.  Now we're more picky about args... */
  ...
  while (argv[optind])
  {
        ...
        while (loport <= curport)
    {
      ...
      Debug(("netfd %d from port %d to port %d", netfd, ourport, curport)) if (netfd > 0) if (o_zero && o_udpmode)
      { /* if UDP scanning... */
#ifdef INET6
        if (want6)
